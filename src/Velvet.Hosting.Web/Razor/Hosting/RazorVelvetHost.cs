...
/// <summary>
/// Razor Pages host for Velvet using canvas-id based initialization.
/// Delegates rendering to VelvetHostCore with animation frame-based loop.
/// </summary>
public sealed class RazorVelvetHost : VelvetHostCore
{
    ...
    private readonly List<(EngineScene Scene, long PreparedFrame)> _scenePreparedFrames = new();
    private long _frameIndex;

    private OrbitInputBinder? _orbitBinder;

    private DotNetObjectReference<RazorVelvetHost>? _callbackRef;
    ...
    public static async Task<RazorVelvetHost> CreateAsync(
        string canvasId,
        IJSRuntime js,
        ...
        IWebGLBridge? bridge = null)
    {
        ...
        var app = new RazorVelvetHost(js, resolvedBridge, rendererId, canvasId)
        {
    ...
    private async Task RenderFrameAsync(double timestampMs)
    {
        ...
        catch (Exception ex)
        {
            if (Volatile.Read(ref _isRunning) == 1)
            {
                ...
                Console.WriteLine($"[Velvet Razor] Frame render faulted: {ex}");
            }
        }
        finally
        {
    ...
    private void PrepareSceneForFrame(EngineScene scene, long frameIndex)
    {
