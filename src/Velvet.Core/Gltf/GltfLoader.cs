...
public static class GltfLoader
{
    ...
    private static Mesh ParseGltf(byte[] jsonUtf8, byte[] binBytes)
    {
        ...
        {
            indices = ReadAccessorIndicesU16(accessors, bufferViews, binBytes, indicesAccessorIndex.Value);
        }
    ...
    private static ushort[] ReadAccessorIndicesU16(JsonElement accessors, JsonElement bufferViews, byte[] bin, int accessorIndex)
    {
        var acc = accessors[accessorIndex];
        var type = acc.GetProperty("type").GetString();
        ...
        {
            5123 => ReadUShortIndices(bin, byteOffset, count),      // UNSIGNED_SHORT
    ...
    private static ushort[] ReadUShortIndices(byte[] bin, int byteOffset, int count)
    {
    ...
    private static ushort[] ReadByteIndices(byte[] bin, int byteOffset, int count)
    {
        var indices = new ushort[count];
        for (var i = 0; i < count; i++)
        {
    ...
    private static ushort[] ReadUIntIndicesAsUShort(byte[] bin, int byteOffset, int count)
    {
        var indices = new ushort[count];
        for (var i = 0; i < count; i++)
        {
            var v = BinaryPrimitives.ReadUInt32LittleEndian(bin.AsSpan(byteOffset + (i * 4), 4));
            if (v > ushort.MaxValue) throw new NotSupportedException("Model has >65535 vertex indices; UNSIGNED_INT indices not supported by this minimal loader.");
            indices[i] = (ushort)v;
        }
        return indices;
    }
